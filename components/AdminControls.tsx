...
function AdminControls() {
  ...
  const drawWinner = async () => {
    ...
    try {
      ...
      console.log('draw winner call failed', error);
  ...
  const onWithdrawCommission = async () => {
    ...
    try {
      ...
      console.log('withdraw commission call failed', error);
  ...
  const onRefundAll = async () => {
    ...
    try {
      ...
      console.log('refund all call failed', error);
  ...
  const onRestartDraw = async () => {
    ...
    try {
      ...
      console.log('draw restart call failed', error);
  ...
  return (
    <div className="text-white text-center px-5 py-3 rounded-md border-emerald-300/20 border">
      ...
      <div className="flex flex-col space-y-2 md:flex-row md:space-y-0 md:space-x-2">
        <button onClick={drawWinner} className="btn-admin">
          <StarIcon className="h-6 mx-auto mb-2"/>
          Draw Winner
        </button>
        <button onClick={onWithdrawCommission} className="btn-admin">
          <CurrencyDollarIcon className="h-6 mx-auto mb-2" />
          Withdraw Commission
        </button>
        <button onClick={onRestartDraw} className="btn-admin">
          <ArrowPathIcon className="h-6 mx-auto mb-2" />
          Restart Draw
        </button>
        <button onClick={onRefundAll} className="btn-admin">
